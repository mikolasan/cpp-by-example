...
std::map<std::string, Show*> cache_shows(std::string config)
{
    ...
    for (ryml::NodeRef const& child : show_mapping.children()) {
        ...
        show_buffer[show_id] = new Show(show_id, fname, hostname);
        show_buffer[show_id]->set_background(background);
        show_buffer[show_id]->set_loop(loop);
        show_buffer[show_id]->set_ignore_stop(ignore_stop);
        show_buffer[show_id]->cache();
    }
...
{
    std::cout << "LED proxy has started" << std::endl;

    std::string contents = get_file_contents("config.yml");
    std::map<std::string, Show*> show_buffer = cache_shows(contents);

    Show* show = nullptr;
    Show* next_show = nullptr;
    std::stack<Show*> background_shows;
        ...
        while(true) {
            {
                ...
                if (background_shows.size() > 1) {
                    Show* background_show = background_shows.top();
                    if (background_show->is_force_stopped()) {
    ...
    auto consumer = std::thread([&backend_listening, &show, &next_show, &background_shows, &show_mutex, &show_buffer](){
        ...
        while (backend_listening) {
            ...
            if (action == "select" || action == "play") {
                ...
                if (action == "select") {
                    // clear the stack
                    std::stack<Show*>().swap(background_shows);
                }
                
                if (show && show_name == show->get_codename() && show->get_background_flag()) {
                ...
                if (show && show_name == show->get_codename()) {
                    ...
                    background_shows.top()->force_stop();
                }
            } else {
