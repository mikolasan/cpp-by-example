...
// Original code: https://github.com/hideakitai/ArtNet
...
namespace artnet {
    ...
    constexpr uint16_t PROTOCOL_VER {14};
    ...
    public:
        ...
        void streaming(const std::string& ip, const uint32_t universe_) {
            // uint32_t now = millis();
        ...
        void streaming(const std::string& ip, const uint8_t net_, const uint8_t subnet_, const uint8_t universe_) {
            uint32_t u = ((uint32_t)net_ << 8) | ((uint32_t)subnet_ << 4) | (uint32_t)universe_;
            streaming(ip, u);
        }

        // one-line sender
        void send(const std::string& ip, const uint32_t universe_, const uint8_t* const data, const uint16_t size) {
            set_universe(universe_);
            streaming_data(data, size);
            send_packet(ip);
        }
        void send(const std::string& ip, const uint8_t net_, const uint8_t subnet_, const uint8_t universe_, const uint8_t* const data, const uint16_t size) {
            set_universe(net_, subnet_, universe_);
            streaming_data(data, size);
            send_packet(ip);
        }

        void set_physical(const uint8_t physical) { phy = physical; }
    ...
    protected:
        ...
        void send_packet(const std::string& ip) {
    ...
    public:
        ...
        OpCode parse() {
            ...
            if (checkID(d)) {
                switch (opcode(d)) {
                    ...
                    default: {
                        std::cout << "Unsupported OpCode: " << std::hex << opcode(d) << std::endl;
                        op_code = OpCode::NA;
                        break;
                    }
                }
        ...
        inline void subscribe(const uint8_t universe, Fn&& func) {
            if (callbacks.size() >= 4) {
                std::cout << "too many callbacks" << std::endl;
            } else {
                if (universe > 0xF) {
                    std::cout << "universe out of bounds" << std::endl;
                    return;
                } else {
                    uint32_t u = ((uint32_t)net_switch << 8) | ((uint32_t)sub_switch << 4) | (uint32_t)universe;
                    callbacks.insert(make_pair(u, func));
                }
            }
        }
        template <typename F>
        inline void subscribe(F&& func) {
            callback_all = func;
        }
        ...
        void shortname(const std::string& sn) {
            short_name = sn;
        }

        void longname(const std::string& ln) {
            long_name = ln;
        }
    ...
    protected:
        void attach(S& s, const uint8_t subscribe_net = 0, const uint8_t subscribe_subnet = 0) {
            ...
            if (subscribe_net > 128) {
                std::cout << "Net must be < 128" << std::endl;
            } else {
                net_switch = subscribe_net;
            }
            if (subscribe_subnet > 16) {
                std::cout << "Subnet must be < 16" << std::endl;
            } else {
    ...
    public:
        ...
        void open(const std::string& fname) {
            stream.open(fname);
            this->Receiver_<S>::attach(stream, 0, 0);
        }
        bool hasPackets() {
            return stream.hasPackets();
        }
        S& getStream() {
            return stream;
        }
